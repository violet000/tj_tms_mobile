import 'dart:convert';
import 'package:convert/convert.dart';
import 'package:crypto/crypto.dart';
import 'package:sm_crypto/sm_crypto.dart';
import 'dart:typed_data';


class MD5Util {
  static String generateMd5(String data) {
    var content = new Utf8Encoder().convert(data);
    var digest = md5.convert(content);
    return hex.encode(digest.bytes);
  }
}

class SM4Context {
  int mode = 1;
  bool isPadding = true;
  List<int> sk = List.filled(32, 0);
}

class SM4 {
  static const int SM4_ENCRYPT = 1;
  static const int SM4_DECRYPT = 0;

  static const List<int> SboxTable = [
    
  ];

  static const List<int> FK = [0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc];
  static const List<int> CK = [
    0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269,
    0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
    0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249,
    0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
    0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229,
    0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
    0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209,
    0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279
  ];

  int GET_ULONG_BE(List<int> b, int i) {
    return ((b[i] & 0xff) << 24) |
        ((b[i + 1] & 0xff) << 16) |
        ((b[i + 2] & 0xff) << 8) |
        (b[i + 3] & 0xff);
  }

  void PUT_ULONG_BE(int n, List<int> b, int i) {
    b[i] = (n >> 24) & 0xFF;
    b[i + 1] = (n >> 16) & 0xFF;
    b[i + 2] = (n >> 8) & 0xFF;
    b[i + 3] = n & 0xFF;
  }

  int ROTL(int x, int n) {
    return ((x << n) | (x >> (32 - n))) & 0xFFFFFFFF;
  }

  int sm4Sbox(int inch) {
    int i = inch & 0xFF;
    return SboxTable[i];
  }

  int sm4Lt(int ka) {
    List<int> a = List.filled(4, 0);
    List<int> b = List.filled(4, 0);
    PUT_ULONG_BE(ka, a, 0);
    b[0] = sm4Sbox(a[0]);
    b[1] = sm4Sbox(a[1]);
    b[2] = sm4Sbox(a[2]);
    b[3] = sm4Sbox(a[3]);
    int bb = GET_ULONG_BE(b, 0);
    int c = bb ^ ROTL(bb, 2) ^ ROTL(bb, 10) ^ ROTL(bb, 18) ^ ROTL(bb, 24);
    return c;
  }

  int sm4F(int x0, int x1, int x2, int x3, int rk) {
    return x0 ^ sm4Lt(x1 ^ x2 ^ x3 ^ rk);
  }

  int sm4CalciRK(int ka) {
    List<int> a = List.filled(4, 0);
    List<int> b = List.filled(4, 0);
    PUT_ULONG_BE(ka, a, 0);
    b[0] = sm4Sbox(a[0]);
    b[1] = sm4Sbox(a[1]);
    b[2] = sm4Sbox(a[2]);
    b[3] = sm4Sbox(a[3]);
    int bb = GET_ULONG_BE(b, 0);
    int rk = bb ^ ROTL(bb, 13) ^ ROTL(bb, 23);
    return rk;
  }

  void sm4_setkey_enc(SM4Context ctx, List<int> key) {
    ctx.mode = SM4_ENCRYPT;
    sm4_setkey(ctx.sk, key);
  }

  void sm4_setkey(List<int> SK, List<int> key) {
    List<int> MK = List.filled(4, 0);
    List<int> k = List.filled(36, 0);
    MK[0] = GET_ULONG_BE(key, 0);
    MK[1] = GET_ULONG_BE(key, 4);
    MK[2] = GET_ULONG_BE(key, 8);
    MK[3] = GET_ULONG_BE(key, 12);
    k[0] = MK[0] ^ FK[0];
    k[1] = MK[1] ^ FK[1];
    k[2] = MK[2] ^ FK[2];
    k[3] = MK[3] ^ FK[3];
    for (int i = 0; i < 32; i++) {
      k[i + 4] = k[i] ^ sm4CalciRK(k[i + 1] ^ k[i + 2] ^ k[i + 3] ^ CK[i]);
      SK[i] = k[i + 4];
    }
  }

  List<int> padding(List<int> input, int mode) {
    if (mode == SM4_ENCRYPT) {
      int p = 16 - (input.length % 16);
      List<int> ret = List.from(input);
      for (int i = 0; i < p; i++) {
        ret.add(p);
      }
      return ret;
    } else {
      int p = input.last;
      return input.sublist(0, input.length - p);
    }
  }

  void sm4_one_round(List<int> sk, List<int> input, List<int> output) {
    List<int> ulbuf = List.filled(36, 0);
    ulbuf[0] = GET_ULONG_BE(input, 0);
    ulbuf[1] = GET_ULONG_BE(input, 4);
    ulbuf[2] = GET_ULONG_BE(input, 8);
    ulbuf[3] = GET_ULONG_BE(input, 12);
    for (int i = 0; i < 32; i++) {
      ulbuf[i + 4] = sm4F(ulbuf[i], ulbuf[i + 1], ulbuf[i + 2], ulbuf[i + 3], sk[i]);
    }
    PUT_ULONG_BE(ulbuf[35], output, 0);
    PUT_ULONG_BE(ulbuf[34], output, 4);
    PUT_ULONG_BE(ulbuf[33], output, 8);
    PUT_ULONG_BE(ulbuf[32], output, 12);
  }

  List<int> sm4_crypt_ecb(SM4Context ctx, List<int> input) {
    if (ctx.isPadding && ctx.mode == SM4_ENCRYPT) {
      input = padding(input, SM4_ENCRYPT);
    }
    int length = input.length;
    List<int> bous = [];
    for (int i = 0; i < length; i += 16) {
      List<int> out = List.filled(16, 0);
      List<int> ins = input.sublist(i, i + 16);
      sm4_one_round(ctx.sk, ins, out);
      bous.addAll(out);
    }
    List<int> output = bous;
    if (ctx.isPadding && ctx.mode == SM4_DECRYPT) {
      output = padding(output, SM4_DECRYPT);
    }
    return output.map((e) => e < 0 ? e + 256 : e).toList();
  }

  List<int> sm4_crypt_cbc(SM4Context ctx, List<int> iv, List<int> input) {
    if (iv == null || iv.length != 16) {
      throw Exception("iv error!");
    }
    if (ctx.isPadding && ctx.mode == SM4_ENCRYPT) {
      input = padding(input, SM4_ENCRYPT);
    }
    int length = input.length;
    List<int> bous = [];
    if (ctx.mode == SM4_ENCRYPT) {
      List<int> tempIv = List.from(iv);
      for (int i = 0; i < length; i += 16) {
        List<int> out = List.filled(16, 0);
        List<int> out1 = List.filled(16, 0);
        List<int> ins = input.sublist(i, i + 16);
        for (int j = 0; j < 16; j++) {
          out[j] = ins[j] ^ tempIv[j];
        }
        sm4_one_round(ctx.sk, out, out1);
        tempIv = List.from(out1);
        bous.addAll(out1);
      }
    } else {
      List<int> tempIv = List.from(iv);
      for (int i = 0; i < length; i += 16) {
        List<int> out = List.filled(16, 0);
        List<int> out1 = List.filled(16, 0);
        List<int> ins = input.sublist(i, i + 16);
        sm4_one_round(ctx.sk, ins, out);
        for (int j = 0; j < 16; j++) {
          out1[j] = out[j] ^ tempIv[j];
        }
        tempIv = List.from(ins);
        bous.addAll(out1);
      }
    }
    List<int> output = bous;
    if (ctx.isPadding && ctx.mode == SM4_DECRYPT) {
      output = padding(output, SM4_DECRYPT);
    }
    return output.map((e) => e < 0 ? e + 256 : e).toList();
  }
}

class SM4Util {
  String secretKey = "11HDESaAhiHHugDz";
  String iv = "";
  bool hexString = false;

  String encryptData_ECB(String plainText) {
    try {
      var sm4 = SM4();
      var ctx = SM4Context();
      ctx.isPadding = true;
      ctx.mode = SM4.SM4_ENCRYPT;
      var keyBytes = stringToByte(secretKey);
      sm4.sm4_setkey_enc(ctx, keyBytes);
      var encrypted = sm4.sm4_crypt_ecb(ctx, stringToByte(plainText));
      return base64.encode(encrypted);
    } catch (e) {
      print(e);
      return '';
    }
  }

  String encryptData_CBC(String plainText) {
    try {
      var sm4 = SM4();
      var ctx = SM4Context();
      ctx.isPadding = true;
      ctx.mode = SM4.SM4_ENCRYPT;
      var keyBytes = stringToByte(secretKey);
      var ivBytes = stringToByte(iv);
      sm4.sm4_setkey_enc(ctx, keyBytes);
      var encrypted = sm4.sm4_crypt_cbc(ctx, ivBytes, stringToByte(plainText));
      return base64.encode(encrypted);
    } catch (e) {
      print(e);
      return '';
    }
  }

  List<int> stringToByte(String str) {
    return utf8.encode(str);
  }

  String byteToString(List<int> arr) {
    return utf8.decode(arr);
  }
}

